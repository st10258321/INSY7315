...
class UserViewModel @Inject constructor(
    ...
    // LiveData for reviews of a service provider
    val serviceReviews = MutableLiveData<List<Review>?>()

    // LiveData for adding a review (success/fail + review data)
    val addReviewStatus = MutableLiveData<Triple<Boolean, String?, Review?>>()

    // LiveData for deleting a review
    val deleteReviewStatus = MutableLiveData<Pair<Boolean, String?>>()

    // LiveData for average rating
    val averageRating = MutableLiveData<Pair<Double, Int>>() // (rating, total reviews)
    ...
    fun addReview(serviceId: String, stars: Int, reviewText: String) {
        reviewRepo.addReview(serviceId, stars, reviewText) { success, message, review ->
            addReviewStatus.postValue(Triple(success, message, review))
        }
    }
    ...
    fun getReviewsForServiceProvider(userId: String) {
        reviewRepo.getReviewsForServiceProvider(userId) { success, message, reviews ->
            if (success) {
                serviceReviews.postValue(reviews)
            } else {
                serviceReviews.postValue(null)
            }
        }
    }
    ...
    fun getAverageRating(userId: String) {
        reviewRepo.getServiceProviderAverageRating(userId) { success, message, avg, total ->
            if (success && avg != null && total != null) {
    ...
    fun deleteReview(reviewId: String) {
        reviewRepo.deleteReview(reviewId) { success, message ->
            deleteReviewStatus.postValue(Pair(success, message))
        }
    }
}